document.addEventListener("DOMContentLoaded", () => {
  ...
  const description_album = document.getElementById("description-album");
  let songs = [];
  let currentIndex = 0;
      ...
      setTimeout(() => {
        ...
        if (data.album) {
          ...
          albumWrap.querySelector("p").textContent =
            ...
            description_album.textContent = data.album.artistBio
        }
          ...
          li.innerHTML = `
            <div class="jp-playlist-item">
              <button class="play-btn" data-index="${i}">
                <i style="font-size:12px;padding:12px;  border-radius: 50%;" class="fa fa-play"></i>
              </button>
              <span style="font-size:17px;">${song.title} (${song.genreName})</span>
            </div>`;
